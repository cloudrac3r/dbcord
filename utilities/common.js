...
module.exports = {
	...
	// Number to time period.
	humanTime: function(number) {
		const factors = [
			["d", 24*60*60*1000],
			["h", 60*60*1000],
			["m", 60*1000],
		...
		for (let factor of factors) {
			if (number > factor[1]) {
				if (output) output += " ";
				let multiplier = Math.floor(number/factor[1]);
				output += multiplier+factor[0];
				number -= factor[1]*multiplier;
			}
		}
		return output;
	},
	// Filter, and sort better results to the front
	smartFilter: function(list, path, filter) {
		let output = [];
		const functions = [
			value => value == filter,
			value => value.toLowerCase().startsWith(filter.toLowerCase()),
		...
		for (let f of functions) {
			...
			for (let i = list.length-1; i >= 0; i--) {
				let value = list[i];
				if (path) for (let fragment of path.split(".")) value = value[fragment];
				if (f(value)) {
					bunch.push([value, list[i]])
				} else {
					spare.push(list[i]);
				}
				list.pop();
			...
			if (bunch.length) {
				bunch = bunch.sort((a, b) => {
					if (a[0] == b[0]) return 0;
					else if (a[0].length != b[0].length) return a[0].length - b[0].length;
					else return a[0] < b[0] ? -1 : 1;
				}).map(row => row[1]);
				output = output.concat(bunch);
			}
			list = spare;
		}
		return output;
